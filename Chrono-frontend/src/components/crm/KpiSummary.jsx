...
const KpiSummary = ({ customers, leads, pipelineValue, activeCampaigns, averageDealValue, winRate, dateRangeLabel, ownerFilterLabel }) => {
    const { t } = useTranslation();

    const formatCurrency = (value) => {
        return `CHF (Schweizer Franken) ${Number(value || 0).toLocaleString("de-CH", { minimumFractionDigits: 0 })}`;
    };
    ...
    return (
        <section className="card-grid summary-grid">
            <article className="card stat-card">
                ...
                <div className="muted tiny">{t("crm.filteredBy", "Gefiltert nach")}: {dateRangeLabel} · {ownerFilterLabel}</div>
            </article>
            <article className="card stat-card">
            ...
            <article className="card stat-card">
                ...
                <div className="muted tiny">{t("crm.avgDeal", "Ø (Durchschnitt) Deal")}: {formatCurrency(averageDealValue)}</div>
            </article>
            <article className="card stat-card">
                ...
                <div className="muted tiny">{t("crm.winRate", "Win-Rate (Abschlussquote)")}: {winRate.toFixed(1)}%</div>
            </article>
        </section>
    );
};
