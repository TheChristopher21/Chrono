...
export const normalizeStock = (entry = {}) => ({
    ...
    reservedQuantity: toNumber(entry.reservedQuantity ?? entry.reservedQty),
    availableQuantity: toNumber(entry.availableQuantity ?? entry.availableQty ?? entry.quantity),
    warehouseBinId: entry.warehouseBinId ?? entry.warehouseBin?.id,
    warehouseBinCode: entry.warehouseBinCode ?? entry.warehouseBin?.code ?? "-",
    lotNumber: entry.lotNumber ?? "-",
    serialNumber: entry.serialNumber ?? "-",
    expirationDate: entry.expirationDate ?? null,
    inventoryStatus: entry.inventoryStatus ?? entry.status ?? "AVAILABLE",
    status: entry.inventoryStatus ?? entry.status ?? PROCESS_STATUS.OPEN,
    buckets: {
        AVAILABLE: toNumber(entry.availableQuantity ?? entry.availableQty ?? entry.quantity),
        RESERVED: toNumber(entry.reservedQuantity ?? entry.reservedQty),
