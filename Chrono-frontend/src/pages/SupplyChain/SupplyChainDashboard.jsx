            ...
            <main className="admin-content">
                <header className="hero-header card compact-hero">
                    <div className="header-text">
                        ...
                        <p className="muted">{l("Phasen 0–6 umgesetzt: Prozesse, Ausnahmezentrum, Audit, Rollen-/Mandantenfähigkeit und KPI-Steuerung.", "Phases 0-6 delivered: process framework, exception center, audit, role/multi-company readiness and KPI steering.")}</p>
                    </div>
                ...
                {loading ? <p className="card">{l("Lädt…", "Loading…")}</p> : (
                    <div className="sc-process-grid">
                        <ProcessWorkspace id="inbound" title={l("Wareneingänge", "Inbound")} subtitle={l("ASN, Dock, QC, Putaway", "ASN, dock, QC, putaway")} rows={inboundRows} columns={[{ key: "order", label: "Order" }, { key: "asn", label: "ASN" }, { key: "dock", label: "Dock" }, { key: "supplier", label: l("Lieferant", "Supplier") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="outbound" title={l("Warenausgänge", "Outbound")} subtitle={l("Wave, Pick, Pack, Ship", "Wave, pick, pack, ship")} rows={outboundRows} columns={[{ key: "order", label: "Order" }, { key: "customer", label: l("Kunde", "Customer") }, { key: "priority", label: l("Priorität", "Priority") }, { key: "dueDate", label: l("Datum", "Date") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="production" title={l("Produktionsaufträge", "Production orders")} subtitle={l("Planung bis Abschluss", "Planning to completion")} rows={productionRows} columns={[{ key: "order", label: "Order" }, { key: "product", label: l("Produkt", "Product") }, { key: "quantity", label: l("Menge", "Quantity") }, { key: "priority", label: l("Priorität", "Priority") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="service" title={l("Serviceeinsätze", "Service cases")} subtitle={l("SLA- und Eskalationsfokus", "SLA and escalation focused")} rows={serviceRows} columns={[{ key: "case", label: l("Fall", "Case") }, { key: "customer", label: l("Kunde", "Customer") }, { key: "owner", label: l("Verantwortlich", "Owner") }, { key: "sla", label: "SLA" }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="returns" title={l("Retouren (RMA)", "Returns (RMA)")} subtitle={l("Prüfen, entscheiden, buchen", "Inspect, decide, post")} rows={returnsRows} columns={[{ key: "rma", label: "RMA" }, { key: "customer", label: l("Kunde", "Customer") }, { key: "reason", label: l("Grund", "Reason") }, { key: "decision", label: l("Entscheidung", "Decision") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="inventory" title={l("Mehrstufiges Lager", "Multi-level warehouse")} subtitle={l("Site/Warehouse/Zone/Aisle/Rack/Bin", "Site/warehouse/zone/aisle/rack/bin")} rows={inventoryRows} columns={[{ key: "sku", label: "SKU" }, { key: "product", label: l("Produkt", "Product") }, { key: "warehouse", label: l("Lager", "Warehouse") }, { key: "zone", label: "Zone" }, { key: "bin", label: "Bin" }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="buckets" title={l("Bestandsarten", "Inventory buckets")} subtitle={l("Verfügbar, reserviert, gesperrt, Transit, QC", "Available, reserved, blocked, transit, QC")} rows={bucketRows} columns={[{ key: "bucket", label: l("Art", "Type") }, { key: "quantity", label: l("Menge", "Quantity") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="exceptions" title={l("Exception Center", "Exception center")} subtitle={l("Verspätung, QC-Blocker, SLA-Verletzung", "Delays, QC blockers, SLA breaches")} rows={exceptionRows} columns={[{ key: "type", label: l("Typ", "Type") }, { key: "severity", label: l("Schwere", "Severity") }, { key: "ref", label: l("Referenz", "Reference") }, { key: "owner", label: l("Owner", "Owner") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="cycle-count" title={l("Cycle Counting", "Cycle counting")} subtitle={l("Regelzählung, Spot Count, 4-Augen-Freigabe", "Planned counts, spot count, 4-eyes approval")} rows={cycleCountRows} columns={[{ key: "plan", label: l("Plan", "Plan") }, { key: "sku", label: "SKU" }, { key: "warehouse", label: l("Lager", "Warehouse") }, { key: "variance", label: l("Differenz", "Variance") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="governance" title={l("Audit & Freigaben", "Audit & approvals")} subtitle={l("Wer/Wann/Was/Warum mit Historie", "Who/when/what/why with history")} rows={governanceRows} columns={[{ key: "entity", label: l("Entität", "Entity") }, { key: "action", label: l("Aktion", "Action") }, { key: "changedBy", label: l("Benutzer", "User") }, { key: "reason", label: l("Grund", "Reason") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                        <ProcessWorkspace id="company" title={l("Multi-Company & Standards", "Multi-company & standards")} subtitle={l("Währung, Sprache, Steuer, Intercompany, Shared WH", "Currency, language, tax, intercompany, shared WH")} rows={companyRows} columns={[{ key: "company", label: l("Gesellschaft", "Company") }, { key: "currency", label: l("Währung", "Currency") }, { key: "language", label: l("Sprache", "Language") }, { key: "tax", label: l("Steuer", "Tax") }, { key: "status", label: l("Status", "Status") }]} timeline={movementTimeline} text={text} />
                    </div>
                )}
            </main>
        </div>
