...
public class ApiPagePermissionInterceptor implements HandlerInterceptor {
    ...
    private static final List<PageRule> RULES = List.of(
            ...
            rule("/api/customers/**",
                    pages(UserPermissionService.PAGE_DASHBOARD, UserPermissionService.PAGE_ADMIN_CUSTOMERS,
                            UserPermissionService.PAGE_ADMIN_PROJECTS, UserPermissionService.PAGE_CRM),
                    pages(UserPermissionService.PAGE_ADMIN_CUSTOMERS, UserPermissionService.PAGE_ADMIN_PROJECTS,
                            UserPermissionService.PAGE_CRM)),
            rule("/api/projects/**",
                    pages(UserPermissionService.PAGE_DASHBOARD, UserPermissionService.PAGE_ADMIN_PROJECTS,
                            UserPermissionService.PAGE_ADMIN_PROJECT_REPORT, UserPermissionService.PAGE_ADMIN_TASKS),
            ...
            // Delivery/auto-sync is not implemented yet and credentials are not stored in a vault.
            // Keep this API closed until both properties are production-ready.
            rule("/api/integrations/**",
                    pages(),
                    pages()),
            // Invoice creation currently posts into an accounting journal without tenant scoping.
            rule("/api/billing/**",
                    pages(),
                    pages()),
            rule("/api/audit/**",
